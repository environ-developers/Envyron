...
class FFTCore(NumericalCore):
    ...
    def gradient(self, density: EnvironDensity) -> EnvironGradient:
        ...
        gradient_g = ReciprocalField(grid=self.reciprocal_grid,
                                     rank=3,
                                     griddata_3d=gradient_g)
...
#        return EnvironGradient(density.grid, density.gradient(), 'gradient')
    ...
    def divergence(self, gradient: EnvironGradient) -> EnvironDensity:
        ...
        imag = 0 + 1j
        divergence_g = np.einsum('l...,l...', self.reciprocal_grid.g,
                                 gradient_g) * imag
        divergence_g = ReciprocalField(grid=self.reciprocal_grid,
    ...
    def laplacian(self, density: EnvironDensity) -> EnvironDensity:
        ...
        laplacian_g = ReciprocalField(grid=self.reciprocal_grid,
    ...
    def hessian(self, density: EnvironDensity) -> EnvironHessian:
        """docstring"""
        density_g = density.fft()
        hessian_g = -np.einsum('i,j', self.reciprocal_grid.g,
                               self.reciprocal_grid.g) * density_g
        hessian_g = hessian_g.reshape(9, *self.grid.nr)
        hessian = EnvironHessian(self.grid, 'hessian')
        for ipol in np.arange(9):
            aux_g = ReciprocalField(grid=self.reciprocal_grid,
                                    griddata_3d=hessian_g[ipol, :, :, :])
            hessian[ipol, :, :, :] = aux_g.ifft(force_real=True)
        return hessian

    def convolution_density(self, density_a: EnvironDensity,
                            density_b: EnvironDensity) -> EnvironDensity:
        """docstring"""
        ...
        convolution_density_g = ReciprocalField(
            grid=self.reciprocal_grid, griddata_3d=convolution_density_g)
        convolution_density = convolution_density_g.iff(force_real=True)
        return convolution_density

    def convolution_gradient(self, density: EnvironDensity,
                             gradient: EnvironGradient) -> EnvironGradient:
        """docstring"""
        density_g = density.fft()
        gradient_g = gradient.fft()
    ...
    def convolution_hessian(self, density: EnvironDensity,
                            hessian: EnvironHessian) -> EnvironHessian:
        """docstring"""
        density_g = density.fft()
        convolution_hessian = EnvironHessian(self.grid, 'convolution_hessian')
        for ipol in np.arange(9):
            aux = DirectField(grid=self.grid,
    ...
    def poisson(self, density: EnvironDensity) -> EnvironDensity:
        ...
        poisson_g = ReciprocalField(grid=self.reciprocal_grid,
    ...
    def grad_poisson(self, density: EnvironDensity) -> EnvironGradient:
        ...
        grad_poisson_g[:, mask] = FPI * density_g[
            mask] * imag * self.reciprocal_grid.g[:,
                                                  mask] / self.reciprocal_grid.gg[
                                                      mask]
        grad_poisson_g = ReciprocalField(grid=self.reciprocal_grid,
                                         rank=3,
